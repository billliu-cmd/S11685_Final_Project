...
class SJMEpisodeDataset(EpisodeDataset):
    ...
    under a fixed score combining:
      ...
      score_i = alpha * cos(feat_tgt,feat_ctx_i)+  beta  * (p_tgt @ A @ p_ctx_i) # A = I by default non-learning cross_attention

    where `feat` is the 6-dim SJM summary vector and `p` is the K-dim SJM soft-posterior. fitted via __init__. 

    New Parameters:
      K, lam, cov_mode, feature_window: SJM hyper-parameters (defaults from JM_DEFAULTS).
      recompute_every: napshot cadence for val/test causal fits (defaults from JM_DEFAULTS).
      alpha, beta: Scalar weights on choosing similar context through feature weights and regime similarities.
    """
    def __init__(
        self,
        panel,
        ...
        *,
        ...
        mode="train",
        seed=42,
        # ---- SJM-specific kwargs (default from JM_DEFAULTS) ----------
        K=None,
        lam=None,
        cov_mode=None,
        feature_window=None,
        ...
        # Step 2 — pre-extract target 6-dim feature vectors.
        ...
        # Step 3 — pre-extract per-candidate 6-dim feature vectors. 
        if mode == "train":
        ...
        # Step 4 — SJM probability extraction
        # Default everything to uniform 1/K.
        self._tgt_prob = np.full((len(self.targets), K), 1.0 / K, dtype=np.float64)

        if mode == "train":
            # One SJM fit on panel truncated to max(train target dates train ctx_pool dates). 
            train_max = max(
                pd.to_datetime(
            ...
            # Snapshot schedule over target_dates ∪ ctx_pool_dates
    ...
    # ---------------------------------------------------------------------
    # Override: deterministic top-C by similarity metric
    # ---------------------------------------------------------------------
    def _pick_ctx_entries(self, tgt_date, idx):
        ...
        # Probability term D — cross_attention
        ...
        if N >= nc:
            ...
            # edge case of not enough candidates
